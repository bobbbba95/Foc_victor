...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     左电机PWM通道初始化
// 参数说明     timer_period        // 定时器周期值
// 参数说明     isr_state           // 是否启用更新中断  0 不启用  1 启用
// 返回参数     void
// 使用示例     motor_left_output_init(PWM_PRIOD_LOAD, 1);
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void motor_left_output_init(uint16 timer_period, uint8 isr_state)
{
    /******************************定义局部参数******************************/
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     左侧电机三相通道 CC0/CC1 配置
// 参数说明     a_duty       A相FOC输出占空比(0~PWM_PRIOD_LOAD)
// 参数说明     b_duty       B相FOC输出占空比(0~PWM_PRIOD_LOAD)
// 参数说明     c_duty       C相FOC输出占空比(0~PWM_PRIOD_LOAD)
// 返回参数     void
// 使用示例     motor_left_cc_set(200, 300, 400);
// 备注信息     先将上一次CC1镜像写入CC0，再将本次duty写入CC1，下一次交换事件生效
//-------------------------------------------------------------------------------------------------------------------
void motor_left_cc_set(uint16 a_duty, uint16 b_duty, uint16 c_duty)
{
    uint16 a_cc1_last = (uint16)Cy_Tcpwm_Pwm_GetCompare1(MOTOR_LEFT_A_PHASE_TCPWM_TIMER);
    uint16 b_cc1_last = (uint16)Cy_Tcpwm_Pwm_GetCompare1(MOTOR_LEFT_B_PHASE_TCPWM_TIMER);
    uint16 c_cc1_last = (uint16)Cy_Tcpwm_Pwm_GetCompare1(MOTOR_LEFT_C_PHASE_TCPWM_TIMER);

    if(a_cc1_last > PWM_PRIOD_LOAD) a_cc1_last = PWM_PRIOD_LOAD;
    if(b_cc1_last > PWM_PRIOD_LOAD) b_cc1_last = PWM_PRIOD_LOAD;
    if(c_cc1_last > PWM_PRIOD_LOAD) c_cc1_last = PWM_PRIOD_LOAD;

    if(a_duty > PWM_PRIOD_LOAD) a_duty = PWM_PRIOD_LOAD;
    if(b_duty > PWM_PRIOD_LOAD) b_duty = PWM_PRIOD_LOAD;
    if(c_duty > PWM_PRIOD_LOAD) c_duty = PWM_PRIOD_LOAD;

    MOTOR_LEFT_A_PHASE_TCPWM_TIMER->unCC0_BUFF.u32Register = PWM_PRIOD_LOAD - a_cc1_last;
    MOTOR_LEFT_B_PHASE_TCPWM_TIMER->unCC0_BUFF.u32Register = PWM_PRIOD_LOAD - b_cc1_last;
    MOTOR_LEFT_C_PHASE_TCPWM_TIMER->unCC0_BUFF.u32Register = PWM_PRIOD_LOAD - c_cc1_last;

    MOTOR_LEFT_A_PHASE_TCPWM_TIMER->unCC1_BUFF.u32Register = (PWM_PRIOD_LOAD + a_duty) / 2;
    MOTOR_LEFT_B_PHASE_TCPWM_TIMER->unCC1_BUFF.u32Register = (PWM_PRIOD_LOAD + b_duty) / 2;
    MOTOR_LEFT_C_PHASE_TCPWM_TIMER->unCC1_BUFF.u32Register = (PWM_PRIOD_LOAD + c_duty) / 2;
    MOTOR_LEFT_A_PHASE_TCPWM_TIMER->unTR_CMD.u32Register  |= 1;  // 配置A相通道交换使能
    MOTOR_LEFT_B_PHASE_TCPWM_TIMER->unTR_CMD.u32Register  |= 1;  // 配置B相通道交换使能
    MOTOR_LEFT_C_PHASE_TCPWM_TIMER->unTR_CMD.u32Register  |= 1;  // 配置C相通道交换使能
}
...
// 返回参数     void
// 使用示例     motor_left_phase_check(300);
// 备注信息     该函数会通过控制不同通道的MOS，使电机发出不同频率的声音，便于测试硬件功能，调用后需重新主动初始化PWM定时器模块  
//-------------------------------------------------------------------------------------------------------------------
void motor_left_phase_check(uint16 delay_time)
{
    uint16 check_period;
    
    uint16 interval_time = 50;
    
    // 响一声  A相上桥 → B相下桥
    check_period = 36000;                       // 2.222khz
    
    motor_left_output_init(check_period, 0);
    
    motor_left_channel_set(1,1,0);
    
    motor_left_duty_set(check_period / 50, 0, 0);
    
    system_delay_ms(interval_time);
...
// 参数说明     isr_state           // 是否启用更新中断  0 不启用  1 启用
// 返回参数     void
// 使用示例     motor_right_output_init(PWM_PRIOD_LOAD, 1);
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void motor_right_output_init(uint16 timer_period, uint8 isr_state)
{
    /******************************定义局部参数******************************/
    cy_stc_gpio_pin_config_t            pwm_pin_config;                         // 定义PWM引脚配置对象
    cy_stc_tcpwm_pwm_config_t           tcpwm_pwm_config;                       // 定义PWM定时器配置对象
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     右侧电机三相通道 CC0/CC1 配置
// 参数说明     a_duty       A相FOC输出占空比(0~PWM_PRIOD_LOAD)
// 参数说明     b_duty       B相FOC输出占空比(0~PWM_PRIOD_LOAD)
// 参数说明     c_duty       C相FOC输出占空比(0~PWM_PRIOD_LOAD)
// 返回参数     void
// 使用示例     motor_right_cc_set(200, 300, 400);
// 备注信息     先将上一次CC1镜像写入CC0，再将本次duty写入CC1，下一次交换事件生效
//-------------------------------------------------------------------------------------------------------------------
void motor_right_cc_set(uint16 a_duty, uint16 b_duty, uint16 c_duty)
{
    uint16 a_cc1_last = (uint16)Cy_Tcpwm_Pwm_GetCompare1(MOTOR_RIGHT_A_PHASE_TCPWM_TIMER);
    uint16 b_cc1_last = (uint16)Cy_Tcpwm_Pwm_GetCompare1(MOTOR_RIGHT_B_PHASE_TCPWM_TIMER);
    uint16 c_cc1_last = (uint16)Cy_Tcpwm_Pwm_GetCompare1(MOTOR_RIGHT_C_PHASE_TCPWM_TIMER);
    ...
    MOTOR_RIGHT_A_PHASE_TCPWM_TIMER->unCC0_BUFF.u32Register = PWM_PRIOD_LOAD - a_cc1_last;
    MOTOR_RIGHT_B_PHASE_TCPWM_TIMER->unCC0_BUFF.u32Register = PWM_PRIOD_LOAD - b_cc1_last;
    MOTOR_RIGHT_C_PHASE_TCPWM_TIMER->unCC0_BUFF.u32Register = PWM_PRIOD_LOAD - c_cc1_last;

    MOTOR_RIGHT_A_PHASE_TCPWM_TIMER->unCC1_BUFF.u32Register = (PWM_PRIOD_LOAD + a_duty) / 2;
    MOTOR_RIGHT_B_PHASE_TCPWM_TIMER->unCC1_BUFF.u32Register = (PWM_PRIOD_LOAD + b_duty) / 2;
    MOTOR_RIGHT_C_PHASE_TCPWM_TIMER->unCC1_BUFF.u32Register = (PWM_PRIOD_LOAD + c_duty) / 2;
    MOTOR_RIGHT_A_PHASE_TCPWM_TIMER->unTR_CMD.u32Register  |= 1;  // 配置A相通道交换使能
    MOTOR_RIGHT_B_PHASE_TCPWM_TIMER->unTR_CMD.u32Register  |= 1;  // 配置B相通道交换使能
    MOTOR_RIGHT_C_PHASE_TCPWM_TIMER->unTR_CMD.u32Register  |= 1;  // 配置C相通道交换使能
}
...
// 备注信息     该函数会通过控制不同通道的MOS，使电机发出不同频率的声音，便于测试硬件功能，调用后需重新主动初始化PWM定时器模块  
//-------------------------------------------------------------------------------------------------------------------
void motor_right_phase_check(uint16 delay_time)
{
    uint16 check_period;
    ...
    motor_right_output_init(check_period, 0);      
